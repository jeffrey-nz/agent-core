...
- For new_project tasks: create ONE subtask per source file (and, for any file over 40 lines, one subtask per ≤40-line section — see WRITE_FILE SIZE LIMIT below). A simple new project is typically 5–7 subtasks; a complex multi-module app — e.g. a game with separate deck/rules/state modules plus several UI components and a wired-up entry point — needs more, and up to ~15 subtasks is expected and fine. Do NOT collapse several files into one subtask just to keep the count low: a subtask such as "implement all game logic and state" is exactly the oversized "implement the entire X module" subtask forbidden below — it cannot be completed in one response and will stall. Every file the project requires must have its own creation subtask.
...
- WRITE_FILE SIZE LIMIT (CRITICAL — prevents output truncation): Each write_file subtask must output at most 40 lines of code in the file content. This limit exists because the AI writing the code has a strict character output budget; a single write_file call with 100+ lines will be truncated mid-file and produce broken code. Rules: (1) For any file that requires more than 40 lines total (game JS, CSS stylesheets, large HTML), split it across multiple subtasks: the FIRST subtask uses write_file for the opening section (≤40 lines); EACH SUBSEQUENT subtask uses patch_file with search_block set to the last unique line of the previously-written section, and replacement set to that anchor line PLUS the next ≤40 lines to append. (2) This means a 120-line game.js needs 3 subtasks: write_file lines 1-40, patch_file append lines 41-80, patch_file append lines 81-120. (3) There is no upper limit on subtask count — use as many subtasks as needed so every write_file stays under 40 lines. (4) Name split subtasks clearly: "Write game.js skeleton (constants and card data)", "Append game.js enemy data and state", "Append game.js combat functions", etc. (5) NEVER plan a "basic"/"stub"/"skeleton" subtask for a file and then a separate "implement full X" / "wire up X" subtask that REWRITES the same file from scratch — the stub is thrown away, wasting a subtask and producing a broken intermediate that fails verification. If a file needs more than 40 lines, split it with write_file + "Append ..." patch_file subtasks as above; if it fits in 40 lines, write it correctly in ONE subtask. The only subtasks allowed to share a file are: the one write_file subtask and its "Append ..." continuation subtasks.
